...
function buildProjectCard(repository) {
	...
	const icon = createElement(
		"i",
		`bx ${getProjectIcon(repository.language)} project-icon`,
	);
	...
	meta.append(createMetaItem("bx-code-alt", repository.language || "Code"));
...
function normalizeRepositories(repositories) {
	...
	const withDescription = filtered.filter((repository) =>
...
export async function initFeaturedProjects() {
	...
	try {
		...
		if (repositories.length === 0) {
			showFeedback(
				error,
				"No public repositories matched the current filters.",
			);
			return;
		}
